...
*******************************************************************************/
#if (CY_PSOC3)
    cystatus USER_INFO_Write(const uint8 srcBuf[], const uint8 eepromPtr[], uint16 byteCount)
...
#endif /* (!CY_PSOC3) */
{
    ...
    #if (CY_PSOC3)
        eeOffset &= USER_INFO_CODE_ADDR_MASK;
    #endif /* (CY_PSOC3) */

    /* Make sure, that eepromPtr[] points to ROM */
    #if (CY_PSOC3)
        if (((uint32)eepromPtr >= USER_INFO_CODE_ADDR_OFFSET) &&
            (((uint32)eepromPtr + byteCount) <= USER_INFO_CODE_ADDR_END))
    #else
        if (((uint32)eepromPtr + byteCount) <= USER_INFO_FLASH_END_ADDR)
    #endif /* (CY_PSOC3) */
    {
        #if (!CY_PSOC4)
            (void)CySetTemp();
        ...
        rowId = (eeOffset / CY_FLASH_SIZEOF_ROW) % USER_INFO_ROWS_IN_ARRAY;
        ...
        while ((srcIndex < byteCount) && (CYRET_SUCCESS == rc))
        {
            ...
            rowId++;
                ...
                if (rowId >= USER_INFO_ROWS_IN_ARRAY)
                {
                    arrayId++;
                    rowId = 0u;
                }
